...
--근데 이러면 order by bdate desc가 안 먹힌다. 

--그래서
--order by와 함수를 함께 사용하는 경우에는 order by부터 먼저 진행하도록 서브쿼리 형식으로 쿼리를 작성.
...
--안의 서브쿼리로 테이블같은 걸 만든다고 생각하기 >> 거기를 기반으로 해서 쿼리를 실행.

--board_content.jsp용 쿼리
select bid,btitle,bcontent, bhits,bdate from cgv_board
where bid = 'b_1';

delete from cgv_board where bid='b_6';
